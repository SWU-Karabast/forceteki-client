...
import {
  updateIdsWithMapping,
  mapIdToInternalName,
  transformDeckWithCardData,
...
const deckOptions: string[] = ["Order66", "ThisIsTheWay"];
...
const CreateGameForm: React.FC<ICreateGameFormProps> = ({
  ...
  const fetchDeckData = async (deckLink: string) => {
    try {
      ...
      const setCodeMapping = await fetch(
        "/api/s3bucket?jsonFile=_setCodeMap.json"
      ...
      const codeInternalnameMapping = await fetch(
        "/api/s3bucket?jsonFile=_cardMap.json"
      ); // Adjust to your actual endpoint if different
      ...
      const deckWithInternalNames = mapIdToInternalName(
        codeInternalnameJson,
        deckWithIds
      );

      // Fetch internalNameToCardMapping
      const finalDeckForm = await transformDeckWithCardData(
        deckWithInternalNames
      );
