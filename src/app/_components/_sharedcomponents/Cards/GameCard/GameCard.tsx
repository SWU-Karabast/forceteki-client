...
const GameCard: React.FC<GameCardProps> = ({ card, isFaceUp = true }) => {
  ...
  return (
    <MuiCard
      sx={{ backgroundColor: cardBorderColor(card) }}
      onClick={() => {
        if (card.selectable) {
          ...
          <CardContent>
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                <Typography
                  variant="body1"
                  sx={{ ...typographyStyle, color: "goldenrod" }}
                >
                  {card.cost}
                </Typography>
                <Typography
                  variant="body1"
                  sx={{ ...typographyStyle, color: "hotpink" }}
                >
                  {card.damage}
                </Typography>
              </Box>
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                <Typography
                  variant="body1"
                  sx={{ ...typographyStyle, color: "red" }}
                >
                  {card.power}
                </Typography>
                <Typography
                  variant="body1"
                  sx={{ ...typographyStyle, color: "blue" }}
                >
                  {card.hp}
                </Typography>
              </Box>
